...
public class HttpClientSynchronous {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
    	...
    	ArrayList<String> nombres = CombinacionesArchivos.leerArchivo("src/nombres.txt");
		ArrayList<String> apellidos = CombinacionesArchivos.leerArchivo("src/apellidos.txt");
		...
		for (String nombre : nombres) {
			for (String ap1 : apellidos) {
				for (String ap2 : apellidos) {
					...
					HttpRequest postRequest = HttpRequest.newBuilder()
			                .uri(URI.create("http://localhost:8080/alumnos"))
			                //.header("Content-Type", "text/plain")
			                .header("Content-Type", "application/json")
			                .header("X-Debug", "true") // tiempo
			                .POST(HttpRequest.BodyPublishers.ofString(json))
			                //.POST(HttpRequest.BodyPublishers.ofString("5,10"))
			                .build();
	                ...
	                System.out.println("Body: " + responsePost.body()+"\n");
	             // print header debug si existe
	                responsePost.headers().firstValue("X-Debug-Info")
	                        .ifPresent(value -> System.out.println("Debug: " + value));
